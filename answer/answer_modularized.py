...
def tokenize_sent(article_file):
    ...
    for paragraph in paragraphs[1:len(paragraphs)]: # Skip the title
        ...
        for sentence_tokenized in p:
            ...
            words = [w for w in sentence_tokenized if re.search('[a-zA-Z1-9]', w)]
            if len(words) <= 4 or (len(words) <= 10 and words_in_p <= 20): #or "." not in sentence:
                continue
...
def cosineSim(sentences_pool, question, question_start, title):
    ...
    question_expand = question
    ques_pos = st_pos.tag(question) 


    for w,t  in ques_pos:
    	try:
	        if t[0].lower() == 'v' and en.verb.present(w) != 'do' and en.verb.present(w) != 'be' and en.verb.present(w) != 'have':
	            question_expand.append(en.verb.past(w))
	            question_expand.append(en.verb.present(w, person=3))
        except:
        	pass
    corpus = [' '.join(question_expand)]
    ...
    if question_start == 'why' or 'result' in question or 'reason' in question:
        new_related_indices = [0]
        if cosine_similarities[related_docs_indices[1]] > 0.4:
            new_related_indices.append(related_docs_indices[1])

        for (i, s) in enumerate(corpus):
            for q_word in ['because', 'since', 'so', 'goal', "why", 'reason', 'result']:
